...
emptyBoard :: Picture
emptyBoard = pictures [tran x y empty | x <- [left .. right], y <- [bottom .. top]]
...
tran x y = translate (36 * xf) (36 * yf)
  where
...
gamePic (Playing (x, y) o b _) =
  pictures
    [ emptyBoard,
      tran x y filled,
      color violet (pictures [tran a b filled | (a, b) <- o]),
      color red (pictures [tran a b filled | (a, b) <- b])
    ]
gamePic Won = blank
gamePic GameOver = text "You Lost"
...
onBoard :: Coord -> Bool
onBoard (x, y) = elem x [left .. right] && elem y [bottom .. top]
...
collide xs ys = (filter (`notElem` ys) xs, filter (`notElem` xs) ys)
...
moveAndCollide move (moving, static) = collide ([move g | g <- moving]) static
...
next t (Playing p o b []) = Won
next t (Playing p o b l)
  | null [(r, t) | (r, t) <- o, t == (-10)] = Playing p on ob (tail l)
  | otherwise = GameOver
  where
    (e, f) = moveAndCollide decY (o ++ [(g - right, top) | g <- head l], b)
    (ob, on) = moveAndCollide incY ([j | j <- f, onBoard j], e)
next t GameOver = GameOver
next t Won = Won

incY, incX, decY, decX :: Coord -> Coord
incY (x, y) = (x, incBound y top)
incX (x, y) = (incBound x top, y)
decY (x, y) = (x, decBound y bottom)
decX (x, y) = (decBound x bottom, y)
...
main =
  play
    (InWindow "UGent Brick Game" (500, 800) (10, 10))
